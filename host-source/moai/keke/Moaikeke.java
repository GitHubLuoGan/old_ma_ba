...
//		AccountAgent.useNewApi = true;
		...
		GameCenterSDK.init(gameCenterSettings, sActivity);
...
;
			...
			final ApiCallback kebiPayment = new ApiCallback() {
				...
				final FixedPayInfo payInfo = new FixedPayInfo(System.currentTimeMillis()
						+ new Random().nextInt(1000) + "", OrderNo, price*100);
				payInfo.setProductDesc(info);
				payInfo.setProductName(sConfigJsonObject.get("proName").asString());
				payInfo.setCallbackUrl(sConfigJsonObject.get("callBack").asString());
				payInfo.setGoodsCount(price*10);
				GameCenterSDK.getInstance().doFixedKebiPayment(kebiPayment, payInfo, sActivity);
				//doNormalKebiPayment(kebiPayment, payInfo,sActivity);			
