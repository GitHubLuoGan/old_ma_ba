...
// MoaiCrittercism
//================================================================//
public class Moaictcc  extends  MoaiBaseSdk {
	...
	public  void SDKInit(String parms){	 	
		ctccsmsPayCodeConfig.setPayCodeConfig(); 		
...
public void ResultChannelInfo(){
        ...
        channelInfo.add("isConfirmExitEnabled", false);     
...
/*****************************MoreGame*******************************/	
	public static String V2_openMoreGame(JsonValue parms){
		try{
			  PackageManager packageManager = MoaiBaseSdk.sActivity.getPackageManager(); 
		       	Intent intent=new Intent(); 
		       	intent =packageManager.getLaunchIntentForPackage("com.egame"); 
		       	if(intent==null){
		       		Intent intent_ = new Intent("android.intent.action.VIEW");
					intent_.setData(Uri.parse("http://play.cn/"));
					MoaiBaseSdk.sActivity.startActivity(intent_);
		       }
	...
	{ 
		try { 
           ...
           // payinfoJson.get("price").asInt();
            ...
            ctccsmsPayCodeConfig bConfig=new ctccsmsPayCodeConfig();
            bConfig.number=number;
            bConfig.money=total;
            bConfig=ctccsmsPayCodeConfig.getPayCodeConfig(bConfig);
	...
	private static void ctccPay(HashMap<String, String> payParams){
		...
		EgamePay.pay(MoaiBaseSdk.sActivity, payParams,new EgamePayListener() {
			...
			public void paySuccess(Map<String, String> params) {
				...
				jsonParms.add("payData",Moaictcc.orderParms.asObject());				 
				//计费成功
